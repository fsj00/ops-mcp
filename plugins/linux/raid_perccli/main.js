function resolveBin(ctx, host, preferred) {
  var names = preferred ? [preferred] : ["perccli64", "perccli"];
    ...
    var r = ctx.ssh.exec({
      ...
      args: [
        ...
        "bin=" +
          ...
          "for d in /opt/MegaRAID/perccli /usr/sbin /usr/local/bin; do " +
  ...
  throw new Error(
    "perccli/perccli64 not found; run linux_raid_detect first"
  );
}
...
function execute(ctx) {
  ...
  switch (action) {
    ...
    default:
      throw new Error(
        "unsupported action: use summary|status|controllers|virtual_drives|physical_drives|all"
      );
  }
  return ctx.ssh.exec({
    host: host,
